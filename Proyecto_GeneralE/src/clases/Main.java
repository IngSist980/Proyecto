...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (castilloJugador.getPuntos() > 0 && castilloCpu.getPuntos() <= 0) {
            sonidoInicio.stop();
            Sonido sonidoVictoria = new Sonido();
            sonidoVictoria.play("Victoria.wav");
            System.out.println("\nEl jugador es el ganador");
        } else if (castilloJugador.getPuntos() <= 0 && castilloCpu.getPuntos() > 0) {
            sonidoInicio.stop();
            Sonido sonidoDerrota = new Sonido();
            sonidoDerrota.play("Perdiste.wav");
            System.out.println("\nEl CPU es el ganador");
        } else if (castilloJugador.getPuntos() <= 0 && castilloCpu.getPuntos() <= 0) {
            sonidoInicio.stop();
            // Puedes agregar un sonido de empate aquí si lo deseas
            System.out.println("\nEl resultado es EMPATE");
        }

// Agrega un breve retardo para permitir que el sonido se reproduzca antes de que el programa termine.
        try {
